...
CREATE TABLE IF NOT EXISTS OGN_DAILY_STATS(     
    date INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS OGN_RECEIVER(	
	...
	max_alt DECIMAL(5,1),
	max_alt_aircraft_id VARCHAR(9),
	max_alt_aircraft_reg VARCHAR(9),
	max_alt_timestamp INTEGER,
	PRIMARY KEY (date,receiver_name)
);

/** views **/
CREATE VIEW IF NOT EXISTS OGN_DAILY_STATS_V 
AS SELECT 
    date(date/1000,'unixepoch','localtime') as date, 
    online_receivers,
    unique_aircraft_ids    
FROM OGN_DAILY_STATS;

CREATE VIEW IF NOT EXISTS OGN_MAX_RANGE_V 
AS SELECT 
    date(date/1000,'unixepoch','localtime') as date, 
    receiver_name, round(range,2) as range, 
...
AS SELECT 
    date(date/1000,'unixepoch','localtime') as date, 
    receiver_name, beacons_received,
    max_alt,
    max_alt_aircraft_id,
    max_alt_aircraft_reg,
    max_alt_timestamp
FROM OGN_RECEIVER;